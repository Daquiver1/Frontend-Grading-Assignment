...
const Sidebar = () => {
    ...
    return (
        <div
        className={`bg-dark-purple min-h-screen p-5 pt-8 ${open ? "w-72" : "w-20"} relative`}>
        ...
        <Link to="/HelpAndSupport">
          <div className="inline-flex mt-4">
            ...
            <h1 className={`text-gray-500 origin-left font-medium text 9xl duration-300 cursor-pointer mt-5 ${!open && "scale-0"}`}>{" "}HELP AND SUPPORT{" "}</h1></div>
        </Link>
